	...
	$.fn.getArea = function(options) {
		...
		$(this).each(function() {
			...
			$(this).on(params.eventInpFocus, function() {
				//这里本来是用ajax去请求的，这里我去掉了，为了打开项目中的html之后能直接拿到数据，我把json作为一个js变量引入进来了。
			...
			$(params.elePage).on(params.eventClick, params.confirmArea, function() {
				var html = params.provinceObj.name + params.defaultSplit + params.cityObj.name + params.defaultSplit + params
					.regionObj.name;
			...
			$(params.elePage).on('touchend', params.scrollItem, function(e) {
				...
				//2020-3-25 jiajun.yan 新增代码部分start
				//可能会有点击的情况，这样的话就没有触发滑动，无法设置params.scrollNum,此时就取上级元素的top，运算一下
				if (params.scrollNum == 0) {
					var top = $(this).css("top");
					var num = top.replace(/[^0-9]/ig, "");
					params.scrollNum = -num / 40;
				}
				//2020-3-25 jiajun.yan 新增代码部分end
			...
			function getAreaContent() {
				var html = '<div class="area-larger-mask" id="areaScrollMask">' +
					...
					'<div class="title-area-control">请选择所在地区</div>' +
			...
			function handleAreaInput() {
				...
				var regionIndex = regionList.indexOf(areaNum[2]) < 0 ? 0 : regionList.indexOf(areaNum[2]);
